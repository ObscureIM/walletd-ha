...
# Obsidian Obsidian-service High-Availability Wrapper

This project is designed to wrap the Obsidian-service process on a *nix system and monitor it for hangups, locks, and etc that cause the wallet to stop responding.
...
It also provides *easy access* to the Obsidian-service RPC API via native [Javascript Promises](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Using_promises).
...
* [Obsidian-service](https://github.com/ObsidianProtocol/Obsidian/releases) v0.8.4 or higher
...
Practically all of the Obsidian-service command line arguments are exposed in the constructor method. Simply include them in your list of options to activate or use them. Default values are defined below. As always, please use values that make sense for your implementation.
...
var wallet = new TurtleService({
  ...
  pollingInterval: 10, // Check to make sure that Obsidian-service is alive every x seconds
  maxPollingFailures: 3, // After the polling checks fail x times, report the Obsidian-service process down
  ...
  containerPassword: false, // The password to your Obsidian-service container file
  logFile: false, // The path to the log file you would like Obsidian-service to keep
...
Starts Obsidian-service and starts monitoring the process.
...
Stops Obsidian-service and halts all monitoring processes.
...
If you really must write text to the actual Obsidian-service console, you can do so with this method. You'll need to parse the output of the *data* event as defined below.
...
This event is fired initially when the underlying Obsidian-service process is detected as being ```alive```. It will also fire when it comes back ```alive``` after the process has been restarted. In addition, it will fire on the websocket connection after a successful authentication if the service is indeed ```alive```.
...
This event is fired when the underlying Obsidian-service process closes, dies, is killed for whatever reason. Usually, when this occurs we want to restart the process with ```wallet.start()```
...
Provides the actual console output of Obsidian-service on a per line basis.
...
This event is fired when we detected that Obsidian-service appears **down**. It means that it is not responding to RPC requests. If you want to make it automatically restart Obsidian-service when this occurs, simply ```wallet.stop()``` in the event.
...
This event is fired every polling cycle. It returns the equivalent of the Obsidian-service *getStatus* API call.
...
This event is fired when Obsidian-service is fully synchronized with the network.
...
Copyright (C) 2018 The Obsidian Developers
